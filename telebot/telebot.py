...
class Bot:
	...
	byeMessage		= "See you!"
	loopInterval	= 1
	newCommand_info = "Command %s registered.\nMake sure the callback function looks like foo(self,userid,params)!\n%s.newCommand_info = True"
			...
			while True:
				try:
					for u in self.__api.newUpdates():
						self.__onUpdate(u)
						u_id = u['update_id']
						msg = 'message' in u and u['message'] or 'edited_message' in u and u['edited_message']
	...
	def __canUserUseComm(self,userid,cmd):
		...
		c = cmd in self.__commands and 'run' in self.__commands[cmd]
		return c and ( self.__users.isAdmin(userid) or (not 'mustbe' in self.__commands[cmd]) or ('mod'==c['mustbe'] and self.__users.isMod(userid)) )
	...
	def __onInlineQuery(self,u):
		if callable(self.answerInlineQuery):
			...
			r =  self.answerInlineQuery(self,update = u,answer = dummy)
			if tuple==type(r):
	...
	def sendMessage(self,userid,msg,markdown = False):
		try: 
			if markdown:
				self.__api.sendMarkdownMessage(userid, msg)
			else:
				self.__api.sendMessage(userid, msg)
