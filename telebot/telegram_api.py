...
class Api:
	...
	def sendMarkdownMessage(self,chat_id,msg):
		if not msg: return
		msg = urllib.parse.quote(msg)
#		print(msg)
		try: urllib.request.urlopen('%sbot%s/sendMessage?chat_id=%s&text=%s&parse_mode=Markdown'%(url,self.__token,chat_id,msg))
		except urllib.error.HTTPError as e:
			if 403==e.code: raise UnsubscribedError("User has unsubscribed.")
			if 400==e.code: raise UserNotFoundError("User not found")
			raise ApiError("Other sendMessage HTTP Error:",e)
